...
let restSpreadAnswers = require('../../src/02-rest-spread');
...
describe('rest/spread', () => {
    ...
    it('you should be able to use the rest operator to apply a transform function on all arguments', () => {
    ...
    it('you should be able to use spread to concatenate two arrays', () => {
        ...
        expect(restSpreadAnswers.mergeArrays.toString().includes('...')).to.be(
            true
        );
        expect(
            restSpreadAnswers.mergeArrays.toString().includes('concat')
        ).to.be(false);
    ...
    it('you should be able to spread an array of strings into an array of chars', () => {
        // making sure the spread operator is used and not concat
        expect(
            restSpreadAnswers.spreadArrayStrings.toString().includes('...')
        ).to.be(true);
        expect(
            restSpreadAnswers.spreadArrayStrings.toString().includes('concat')
        ).to.be(false);
