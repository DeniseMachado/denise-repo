...
let destructuringAnswers = require('../../src/03-destructuring');
...
describe('destructuring', () => {
    it('you should be able to use destructuring to discard non interesting properties', () => {
        ...
        expect(
            destructuringAnswers.discardProperties.toString().includes('[')
        ).to.be(false);

        let result = destructuringAnswers.discardProperties({
            a: 1,
            b: 2,
            chunk,
            compact,
        ...
        result = destructuringAnswers.discardProperties({
            a: 1,
            b: 2,
            compact,
        });
    ...
    it('you should be able to destruct methods from native objects)', done => {
        const code = destructuringAnswers.getSubstr.toString();

        expect(code.includes('[')).to.be(false);
        expect(code.indexOf('String.prototype')).to.equal(
            code.lastIndexOf('String.prototype')
        );
